...
const mk = (id: string, num: number, title: string, titleAr: string, level: string, freq: string, tags: string[], summary: string, detailed: string, analogy: string, kp: string[], code: string, qs: any[], want: string[], red: string[], green: string[], pre: string[], nid: string, nt: string, rel: any[], pit: string, fix: string, hook: string, cheat: string, cos: string[], pop: string, sal: string): Card => ({ id, number: num, title, titleAr, level: level as any, frequency: freq as any, tags, definition: { summary, detailed, analogy, keyPoints: kp, codeExample: { language: 'dart', code } }, questions: qs, interviewerMind: { whatTheyWant: want, redFlags: red, greenFlags: green }, linkedCards: { prerequisites: pre, nextSteps: [{ id: nid, title: nt }], related: rel }, commonPitfalls: [{ mistake: pit, whyWrong: 'شائع', correctApproach: fix, egyptianContext: 'مهم' }], answerStrategy: { structure: ['تعريف', 'مثال', 'مقارنة'], timeAllocation: { junior: '2 دق', mid: '3 دق', senior: '5 دق' }, keyPhrases: [id] }, quickRevision: { bulletPoints: kp.slice(0, 4), memoryHook: hook, cheatSheet: cheat }, companyTags: cos, egyptianMarket: { popularity: pop, salaryImpact: sal } });
const q = (t: string, q: string, qa: string, d: number, p: string[], time = '2-3 دق') => ({ type: t as any, question: q, questionAr: qa, difficulty: d, expectedAnswer: { points: p, timeToAnswer: time } });
  ...
  build-android:
    ...
    steps:
      ...
      - uses: actions/upload-artifact@v3
        ...
        ['Valeo', 'International', 'ITWorx'], 'Medium', 'Major'),
...
lane :deploy_ios do
  ...
  deliver(submit_for_review: false, force: true)
        ...
        ['Valeo', 'ITWorx', 'International'], 'Low', 'Major'),
...
class AppDatabase extends _\$AppDatabase {
  ...
  Stream<List<Dress>> watchAvailableDresses() =>
    (select(dresses)..where((d) => d.available.equals(true))).watch();
        ...
        ['MaxAB', 'ITWorx', 'Fawry'], 'Medium', 'Major'),
...
version: 1.2.0+5  # versionName+versionCode`,
        [
            ...
            q('System Design', 'Design a release process for a team-managed Flutter app', 'صمم release process لتطبيق Flutter يديره فريق', 4, ['Git tags for releases', 'Changelogs per version', 'Staged rollout percentages']),
        ...
        ['sqlite-drift'], 'swvl-questions', 'Swvl Interview Questions', [],
