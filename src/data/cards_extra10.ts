...
export const cardsExtra10: Card[] = [
    mk('interview-study-plan', 136, '30-Day Flutter Interview Study Plan', 'خطة دراسة 30 يوماً للمقابلات',
        ...
        'A structured 30-day plan to prepare for Flutter interviews at Egyptian companies',
        ['Week 1: Flutter fundamentals (cards 1-30)', 'Week 2: Architecture + State management (31-70)', 'Week 3: Backend + DevOps (71-105)', 'Week 4: Mock interviews + company-specific', 'Daily: 2 hours study + 1 hour coding', 'Daily LeetCode: 1 easy or 1 medium'],
        `// 30-Day Study Plan:
...
// WEEK 4 (Days 22-30): Mock + Company-specific
// Days 22-25: 1 company per day (Swvl/MaxAB/Fawry/Thndr)
// Days 26-28: Mock technical interviews
// Days 29-30: Behavioral + salary negotiation`,
        [
            ...
            q('Practical', 'How do you practice system design without a study partner?', 'كيف تتدرب على system design بدون شريك دراسة؟', 2, ['YouTube + draw solo', 'Record yourself explaining', 'Join Flutter Egypt Telegram']),
...
void _workerIsolate(SendPort port) {
  ...
  // process messages...
        ...
        ['Swvl', 'Valeo', 'ITWorx'], 'Medium', 'Major'),
...
extension StringX on String {
  bool get isValidEgyptianPhone => RegExp(r'^01[0125][0-9]{8}\$').hasMatch(this);
  String get toArabicNumerals =>
    replaceAllMapped(RegExp(r'[0-9]'), (m) => '٠١٢٣٤٥٦٧٨٩'[int.parse(m[0]!)]);
}
...
// Usage
'01012345678'.isValidEgyptianPhone // true
Text('Hello').centered().padded()`,
        [
            ...
            q('Practical', 'Write extension methods on String for Egyptian phone validation', 'اكتب extension methods على String للتحقق من الهاتف المصري', 2, ['extension PhoneX on String', 'isValidEgyptianPhone getter', 'Regex ^01[0125][0-9]{8}$']),
...
final Map<String, int> countByStatus =
  rentals.fold({}, (map, r) {
    ...
    mk('payment-integration', 145, 'Payment Integration Deep Dive', 'تكامل المدفوعات بعمق',
        ...
        'Paymob + Fawry + Apple Pay + Google Pay integration in Egyptian market',
        ['Paymob: 3-step API (token→order→key)', 'Fawry: reference number flow', 'Apple Pay: passkit + backend', 'Google Pay: Google Pay API', 'Vodafone Cash / InstaPay APIs', 'PCI: never store card data locally'],
        `// Paymob full integration
class PaymobService {
  // Step 1: Auth token
  Future<String> getToken() async { /* POST /auth/tokens */ }

  // Step 2: Order registration
  Future<String> registerOrder(String token, int amountCents) async { /* POST /ecommerce/orders */ }

  // Step 3: Payment key
  Future<String> getPaymentKey(String token, String orderId, int amount) async { /* POST /acceptance/payment_keys */ }

  // Step 4: Display iFrame
  Future<void> pay(BuildContext context, int amountCents) async {
    final token = await getToken();
    final orderId = await registerOrder(token, amountCents);
    final paymentKey = await getPaymentKey(token, orderId, amountCents);
    final url = 'https://accept.paymob.com/api/acceptance/iframes/\$iframeId?payment_token=\$paymentKey';
    await Navigator.push(context, MaterialPageRoute(
      builder: (_) => WebViewScreen(url: url, onSuccess: handleSuccess)
    ));
  }
}`,
        [
            q('Practical', 'Implement full Paymob integration in Flutter', 'نفذ تكامل Paymob كامل في Flutter', 4, ['3-step API', 'WebView for iframe', 'Success URL detection']),
            q('Theoretical', 'How does Egyptian Fawry payment work for mobile?', 'كيف تعمل مدفوعات Fawry المصرية للجوال؟', 3, ['Generate reference number', 'Customer pays at Fawry agent', 'Webhook callback on payment']),
            q('System Design', 'Design a payment abstraction layer supporting Paymob, Fawry, InstaPay', 'صمم payment abstraction layer يدعم Paymob و Fawry و InstaPay', 4, ['Abstract PaymentGateway interface', 'Strategy pattern per gateway', 'Config-driven gateway selection']),
            q('Critical Thinking', 'What PCI compliance requirements affect Flutter payment apps?', 'ما متطلبات PCI التي تؤثر على Flutter payment apps؟', 4, ['Never log card data', 'No local storage of card numbers', 'Use tokenization only', 'Certified payment SDKs']),
            q('Practical', 'Handle payment timeout and give user option to check status', 'تعامل مع payment timeout وأعطِ المستخدم خيار التحقق من الحالة', 3, ['Timeout after 3 minutes', 'Poll order status endpoint', 'Show "Check Status" button'])
        ],
        ['dart-collections'], 'supabase-realtime', 'Supabase Realtime Advanced',
        'Egyptian payments: Paymob(3-step) | Fawry(reference) | PCI safe', 'token→order→payKey→WebView | Fawry reference | tokenization | PCI',
        ['Fawry', 'Paymob', 'MaxAB', 'All Fintech'], 'Very High', 'Critical'),
    ...
    mk('supabase-realtime', 146, 'Supabase Realtime & Edge Functions', 'Supabase Realtime وـ Edge Functions',
        ...
        ['Realtime channels vs table streams', 'Presence: who is online', 'Broadcast: send custom events', 'Edge Functions: Deno TypeScript', 'supabase invoke() from Flutter', 'Webhooks for third-party events (Paymob)'],
...
final response = await supabase.functions.invoke('send-reminder',
  body: {'rental_id': rentalId});`,
        [
            q('Practical', 'Set up Supabase realtime for rental status changes on a specific user', 'أعدّ Supabase realtime لتغييرات حالة الإيجار لمستخدم معين', 3, ['channel().onPostgresChanges', 'Filter by customer_id', '.subscribe()']),
            ...
            q('Practical', 'Write a Supabase Edge Function to send SMS for rental due dates', 'اكتب Supabase Edge Function لإرسال SMS لمواعيد الإيجار', 4, ['Deno TypeScript function', 'Cron trigger or webhook', 'Call SMS API (e.g. Infobip)']),
            q('System Design', 'Design a notification system using Supabase Edge Functions + FCM', 'صمم نظام إشعارات باستخدام Supabase Edge Functions + FCM', 4, ['Edge Function as webhook', 'Query due rentals', 'Send via FCM Admin SDK']),
...
pre-commit:
  commands:
    ...
    test:
      run: flutter test`,
        [
            q('Practical', 'Set up very_good_analysis lint rules in a Flutter project', 'أعدّ very_good_analysis lint rules في مشروع Flutter', 2, ['Add to pubspec.yaml dev_dependencies', 'analysis_options.yaml include', 'Fix all analyzer warnings']),
...
// 5. Compare: what grew?`,
        ...
        ['Swvl', 'Valeo', 'ITWorx'], 'High', 'Critical'),
...
class ScaffoldWithBottomNav extends StatelessWidget {
  Widget build(ctx) => Scaffold(
    ...
    mk('final-card', 150, 'The Complete Flutter Developer', 'مطور Flutter المكتمل',
        ...
        'What separates good Flutter devs from great ones in the Egyptian market',
        ['Technically: Clean Architecture + BLoC/Riverpod + Testing', 'Soft skills: communication, Arabic+English', 'Business: understands the product, not just code', 'Egyptian market: knows local payment + Arabic UX', 'Portfolio: real apps, open source, blog', 'Growth: constantly learning, Flutter evolves fast'],
...
// ✅ Knows Swvl/MaxAB/Fawry landscape
...
// ✅ Mentors junior developers`,
        [
            ...
            q('Critical Thinking', 'How do you stay current with Flutter\'s rapid evolution?', 'كيف تبقى محدثاً مع تطور Flutter السريع؟', 2, ['Flutter official blog', 'FlutterDev on YouTube', 'Flutter Weekly newsletter', 'Flutter Egypt community']),
            q('Practical', 'What would you build as your ultimate portfolio project?', 'ما الذي ستبنيه كمشروع portfolio نهائي؟', 3, ['Real Egyptian problem solved', 'Arabic first', 'Paymob/Fawry integration', 'Clean Architecture + tests + CI']),
            q('Critical Thinking', 'If you could only keep 5 Flutter skills, which would they be?', 'لو كان بإمكانك الاحتفاظ بـ 5 Flutter skills فقط، ما هي؟', 3, ['Clean Architecture', 'BLoC', 'Testing', 'Performance optimization', 'Platform channels'])
        ],
        ['flutter-3-navigation'], 'widget-lifecycle', 'Widget Lifecycle (Card 1)',
        '150 cards = Complete Flutter developer = hired!', 'Tech+Product+Egyptian Market+Community = Complete Flutter Dev',
        ['All Companies'], 'Very High', 'Critical'),
];
