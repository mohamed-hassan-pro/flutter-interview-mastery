...
const mk = (
    ...
    companies: string[], popularity: string, salaryImpact: string
): Card => ({
    ...
    commonPitfalls: [{ mistake: pitfall, whyWrong: 'أخطأ شائع', correctApproach: pitfallFix, egyptianContext: 'مهم في المقابلات' }],
    ...
    egyptianMarket: { popularity, salaryImpact }
});
...
Container(key: UniqueKey(), color: color)`,
        ...
        ['MaxAB', 'Swvl', 'ITWorx'], 'Medium', 'Moderate'),
...
final user = AuthState.of(context).user;`,
        ...
        ['ITWorx', 'Valeo', 'Thndr'], 'Medium', 'Major'),
...
OrientationBuilder(builder: (ctx, orientation) {
  ...
  return GridView.builder(gridDelegate:
    SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: cols));
        ...
        ['All Companies'], 'Very High', 'Moderate'),
...
ExcludeSemantics(child: Icon(Icons.decorative, color: Colors.grey))`,
        ...
        ['ITWorx', 'Valeo', 'International'], 'Medium', 'Major'),
...
final result = await client.mutate(MutationOptions(
  ...
  variables: {'name': 'Ali'},
        ...
        ['International', 'Thndr'], 'Low', 'Moderate'),
  ...
  builder: (ctx, snap) {
    ...
    return MessageBubble(snap.data!);
        ...
        [
            ...
            q('System Design', 'Design the backend for Swvl-style live bus tracking', 'صمم backend لـ live bus tracking مثل Swvl', 5, ['WS for driver→server', 'Server broadcast to passengers', 'Passenger WS subscription by route']),
