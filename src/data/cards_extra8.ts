...
const mk = (id: string, num: number, title: string, titleAr: string, level: string, freq: string, tags: string[], summary: string, kp: string[], code: string, qs: any[], pre: string[], nid: string, nt: string, hook: string, cheat: string): Card => ({
  ...
  commonPitfalls: [{ mistake: 'تجاهل ' + title, whyWrong: 'ضروري', correctApproach: 'تعلّم ' + title }],
...
class PaymentProcessorA implements PaymentStrategy {
  @override Future<bool> pay(double amount) async { /* API call */ return true; }
}

class PaymentProcessorB implements PaymentStrategy {
  @override Future<bool> pay(double amount) async { /* API call */ return true; }
}
...
class AddItemCommand implements Command {
  final Item item;
  final Repo repo;
  AddItemCommand(this.item, this.repo);
  @override void execute() => repo.add(item);
  @override void undo() => repo.delete(item.id);
}`,
    [
      q('Practical', 'Implement Strategy pattern for multiple providers', 'نفذ Strategy pattern لمزودين متعددين', 3, ['Abstract Strategy', 'Specific implementations', 'Switch strategy at runtime']),
...
# flutter run --profile --trace-skia`,
  ...
  mk('responsive-adaptive', 112, 'Responsive vs Adaptive Design', 'التصميم المتجاوب مقابل التكيفي',
    ...
    ['Responsive: fluid layouts, LayoutBuilder', 'Adaptive: platform-specific components', 'Adaptive uses native components', 'CupertinoSwitch on iOS, Switch on Android', 'Go_router + ShellRoute for adaptive navigation', 'adaptive_scaffold package'],
    `// Responsive layout
...
Widget buildSwitch(bool value, ValueChanged<bool> onChanged) {
  return Platform.isIOS
    ...
    [
      ...
      q('Critical Thinking', 'When is it better to build separate apps vs one adaptive app?', 'متى يكون بناء apps منفصلة أفضل من app تكيفي واحد؟', 3, ['Separate: very different UX needs', 'Adaptive: same feature set, different layout', 'Same app with adaptive layout'])
    ],
...
scripts:
  ...
  build:
    ...
    [
      ...
      q('Deep Dive', 'How does melos link local packages?', 'كيف يربط melos الـ packages المحلية؟', 4, ['Modifies pubspec.yaml path dependencies', 'melos bootstrap', 'Similar to pub publish local'])
    ],
...
final data = await api.get(); // what if throws?`,
  ...
  mk('interview-final-prep', 115, 'Final Interview Preparation', 'التحضير النهائي للمقابلة',
    ...
    ['Research the company thoroughly', 'Prepare 3 good questions to ask', 'Sleep 8 hours before', 'Arrive 10 minutes early', 'Follow-up email within 24 hours'],
...
// - "How many vacation days?" (timing is wrong)`,
    [
      ...
      q('Practical', 'What should you wear to a Flutter developer interview?', 'ماذا ترتدي في مقابلة Flutter؟', 1, ['Smart casual (jeans + button shirt)', 'Research company culture on LinkedIn', 'For corporate companies: more formal'])
    ],
